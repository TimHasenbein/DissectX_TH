...
fastqc $RAWdir/$SAMPLE"_L"*.fastq.gz -o $RESULTS"/01_fastqc"
echo -e '...done!'
...
fastq_F=`ls $RAWdir | grep $SAMPLE.*R1.fastq.gz | awk -v sd=$RAWdir -v ORS="," '{print sd"/"$1}' | sed 's/,$//'`
fastq_R=`ls $RAWdir | grep $SAMPLE.*R2.fastq.gz| awk -v sd=$RAWdir -v ORS="," '{print sd"/"$1}' | sed 's/,$//'`
...
htseq-count -s no -r pos -f bam $RESULTS"/02_STAR_alignment"/$SAMPLE".bam" ${ANNOTATION} > $RESULTS"/03_HTseq_counts/"$SAMPLE"_unstranded.count"
echo -e '...done!'

##*******************************##
##  Calculate TPMs for placenta  ##
##*******************************##
## Project: dissectX
## Tim Hasenbein
## Last modification 01.2022
## Creation: 10.2021
## output TPM table for placenta
library(tidyverse)
library(ggplot2)
library(biomaRt)
library(data.table)
...
TPM_table <- as.data.frame(apply(count_table[2:57], 2, function(x) tpm(x, genes$Length)))  # adapt for new data (length -1 which is size)
...
######------ Write output  ------######
write.table(TPM_table, paste0(output,"TPM_table_placenta_replicates.txt"), quote=FALSE, sep = "\t", col.names = TRUE, row.names =FALSE)


######------ Calculate TPM means  ------######
TPM_table <- fread(paste0(output,"TPM_table_placenta_replicates.txt"),header = T)
TPM_groups <- pivot_longer(TPM_table,cols=8:63,names_to="sample",values_to="TPMs")
...
# WT samples
TPM_groups$sample <- gsub("_CxD_\\+\\+","_CxWT",TPM_groups$sample)
TPM_groups$sample <- gsub("_CxDF_\\+\\+","_CxWT",TPM_groups$sample)
TPM_groups$sample <- gsub("_CxFi_\\+\\+","_CxWT",TPM_groups$sample)

TPM_groups$sample <- gsub("_FixC_\\+\\+","_WTxC",TPM_groups$sample)
TPM_groups$sample <- gsub("_DFxC_\\+\\+","_WTxC",TPM_groups$sample)
TPM_groups$sample <- gsub("_DxC_\\+\\+","_WTxC",TPM_groups$sample)
...
write.table(mean, paste0(output,"TPM_table_placenta_means.txt"), quote=FALSE, sep = "\t", col.names = TRUE, row.names =FALSE)

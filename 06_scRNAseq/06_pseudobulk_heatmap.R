...
## Creation: 05.2023
## Run Allelome.PRO for scRNA pseudobulk data --> make bam file for all
## Cells with the same Xa status
...
######------  set environment  ------###### 
input <- "./05_AP_pseudobulk/"
output <- "./05_AP_pseudobulk/"
...
sample_names <- gsub(paste0(input,"2023_08_03_"),"",sample_names)
...
res <- res%>%dplyr::select(name,sample,allelic_ratio)
...
plot <- plot%>%dplyr::select(WT_cast_Xa,KO_cast_Xa,WT_bl6_Xa,KO_bl6_Xa)
...
plot_delta <- res%>%dplyr::select(name,delta_Xi,delta_Xa)
